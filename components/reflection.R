...
colorize_text_accurate_and_slow_for_reflection <- function(text, word_scores){
  ...
  df <- 
    ...
    # here you can specify the color/word combinations you need 
    mutate(value2 = dplyr::case_when(
      abs(avg_score) < 0.01 ~ paste0('<span style = "color:black" data-toggle="tooltip" data-placement="bottom" title="Lexicon: SU and NS', " (", original_ngrams, ")",'">', value, '</span>'),
      avg_score < 0 ~ paste0('<span style = "color:blue" data-toggle="tooltip" data-placement="bottom" title="Lexicon: NS', " (", original_ngrams, ")",'">', value, '</span>'),
      avg_score > 0 ~ paste0('<span style = "color:red" data-toggle="tooltip" data-placement="bottom" title="Lexicon: SU', " (", original_ngrams, ")",'">', value, '</span>'),
      TRUE ~ value)) %>%
    select(., -value)
...
reflectionEngineWidget <- function(id, label = "reflectionUI") {
  ns <- NS(id)
  ...
  return(
    fluidPage(
      ...
      fluidRow(
        ...
        column(12,
               box(width = 12, title = strong("Classification Reflection - Examples from The Validation Set"),
                   # column(7,
...
#' 
#' @return NULL
reflectionEngine <- function(input, output, session, work_env, reactive_values_from_other_modules){
  ...
  output$classificationErrorAnalisys <- renderUI({
    ...
    model_run_info <- all_model_infos %>% filter(iteration_number == input$iteration_select_for_inspection & model_type == 'Unsupervised - Lexicon')
    ...
    colorized_text <- lapply(results$fusion.text, function(x){
      list(
        HTML("<p>", "&#8227;", colorize_text_accurate_and_slow_for_reflection(x, selected_words()), "</p>")
      )
    })
  ...
  output$exportValidation <- downloadHandler(
    ...
    content = function(file) {
      ...
      write.xlsx(filtered_assessment_set() %>% select(-"preprocessed_text_for_classification", -"data_designation", -"fusion.text", -"fusion.label"), file, row.names = FALSE)
    }
  )
  ...
  observeEvent({
    ...
    all_model_infos <- getAllModelsRunInfos(current_project_id())
    model_info <- all_model_infos %>% filter(iteration_number == input$iteration_select_for_inspection & model_type == 'Unsupervised - Lexicon')
  ...
  output$termSummaryTable <- DT::renderDataTable({
    ...
    return(datatable(output_table, rownames = FALSE))
  },
  server = TRUE)
#  extensions="Responsive")
  ...
  output$footnote_for_classification_error_analysis <- renderUI({
    ...
    return(
      HTML(paste0("<p>&bull; Terms colored in <span style='color:red;'>red</span> were classified by you as '", work_env$classification_labels$positive, "', while terms in <span style='color:blue;'>blue</span> were classified by you as '", work_env$classification_labels$negative, "'. Terms without a color (i.e., in black font) were not mentioned in the lexicon.</p>"))
    )
  })
  ...
  output$modelStats <- renderUI({
    ...
    all_model_infos <- getAllModelsRunInfos(current_project_id())
    cur_model <- all_model_infos %>% filter(iteration_number == input$iteration_select_for_inspection & model_type == 'Unsupervised - Lexicon')
    if (nrow(all_model_infos)==0){
      return(NULL)
    }
  ...
  feedbackToHTML <- function(){
    ...
    code <- paste0("<!DOCTYPE html>
              <html>
                <body>
                  <h1>Iteration ", input$iteration_select_for_inspection, " Feedback - Reflection [", Sys.Date(), "]</h1>",
  ...
  output$exportFeedback <- downloadHandler(
    filename = function() {
      #paste("validation_set_", Sys.Date(), ".csv", sep = "")
      paste("reflection_feedback_", Sys.Date(), ".html", sep = "")
    },
    content = function(file) {
      #write.csv(filtered_assessment_set() %>% select(-"preprocessed_text_for_classification", -"data_designation", -"fusion.text", -"fusion.label"), file, row.names = FALSE)
