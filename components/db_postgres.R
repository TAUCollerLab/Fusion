...
initializeDBTables <- function(db_connection){
  ...
  db_response <- dbExecute(db_connection, "INSERT INTO users (full_name, email) VALUES ('Tammy', 'tammy@fakemail.com')")
  db_response <- dbExecute(db_connection, "INSERT INTO users (full_name, email, role) VALUES ('Adi', 'adi@stubemail.com', 2)")
  ######################################
  ...
  db_response <- dbExecute(db_connection, "CREATE TABLE IF NOT EXISTS data (
                            ...
                            serialized_enhancement_corpus BYTEA,
                            positive_label TEXT,
                            negative_label TEXT,
                            created_at TIMESTAMP DEFAULT (NOW() at time zone 'UTC')
                        )")
  ...
  db_response <- dbExecute(db_connection, 'ALTER TABLE "lexicons" ADD FOREIGN KEY ("project_id") REFERENCES "projects" ("id")')
  db_response <- dbExecute(db_connection, 'ALTER TABLE "concepts" ADD FOREIGN KEY ("project_id") REFERENCES "projects" ("id")')
  ...
  db_response <- dbExecute(db_connection, "CREATE TABLE IF NOT EXISTS model_run_info (
                            ...
                            serialized_assessment_set BYTEA,
                            serialized_word_scores BYTEA,
                            lexicon_id INTEGER,
                            created_at TIMESTAMP DEFAULT (NOW() at time zone 'UTC'),
...
getUserInfo <- function(user_id){
  ...
  user_info <- dbGetQuery(appDB, paste("SELECT id, full_name, email, role, (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Jerusalem') as created_at
...
getDataByID <- function(data_id){
  ...
  serialized <- dbGetQuery(appDB, paste("SELECT id, serialized_data, serialized_enhancement_corpus, positive_label, negative_label, (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Jerusalem') as created_at
...
getLexiconByID <- function(lexicon_id){
  ...
  serialized <- dbGetQuery(appDB, paste("SELECT id, serialized_lexicon, project_id, (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Jerusalem') as created_at
...
doesProjectDataExist <- function(project_id){
  ...
  if (length(project_info$data_id)==0){
    return(FALSE)
  }
...
getAllLexiconsInProject <- function(project_id){
  ...
  lexicons_in_project <- dbGetQuery(appDB, paste("SELECT id, serialized_lexicon, project_id, (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Jerusalem') as created_at FROM lexicons WHERE project_id = ", project_id, sep = ""))
...
setProjectData <- function(project_id, work_env){
  ...
  sql_query <- paste("INSERT INTO data (serialized_data, serialized_enhancement_corpus, positive_label, negative_label) VALUES (?, ?, ?, ?) RETURNING id",
                     sep = "")
  inserted_row_id <- dbGetQuery(appDB, sql_query, params = serialized)
  
  # updates the 'projects' table to be aware that a new data has been assigned to this project
  db_response <- dbExecute(appDB, paste("UPDATE projects SET data_id = ", inserted_row_id$id, " WHERE id = ", project_id, sep = ""))
  return(inserted_row_id$id)
}
...
addLexiconToProject <- function(project_id, lexicon){
  ...
  # inserts the data in 'data' table
  sql_query <- paste("INSERT INTO lexicons (serialized_lexicon, project_id) VALUES (?, ?) RETURNING id",
                     sep = "")
  inserted_row_id <- dbGetQuery(appDB, sql_query, params = serialized)
  # updates the 'projects' table to be aware that new data has been assigned to this project
  switchProjectLexicon(project_id, inserted_row_id$id)
  return(inserted_row_id$id)
}
...
storeModelRunInfo <- function(model_id, iteration_number, auc, leave_out_auc, confusion_matrix, assessment_set, word_scores, lexicon_id){
  ...
  sql_query <- paste("INSERT INTO model_run_info (model_id, iteration_number, auc, leave_out_auc, serialized_confusion_matrix, serialized_assessment_set, serialized_word_scores, lexicon_id) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                     sep = "")
  db_response <- dbExecute(appDB, sql_query, params = model_info)
  return(NULL)
}
...
storeModel <- function(user_id, project_id, model_type, model){
  ...
  sql_query <- paste("INSERT INTO models (user_id, project_id, model_type, serialized_model) 
                     VALUES (?, ?, ?, ?) RETURNING id",
                     sep = "")
  inserted_row_id <- dbGetQuery(appDB, sql_query, params = model_info)
  return(inserted_row_id$id)
}
...
updateModel <- function(model_id, model){
  ...
  sql_query <- paste("UPDATE models SET serialized_model = ? WHERE id = ? RETURNING id",
                     sep = "")
  inserted_row_id <- dbGetQuery(appDB, sql_query, params = new_model_info)
  return(inserted_row_id$id)
}
...
getModelRunInfo <- function(model_id){
  ...
  sql_query <- paste("SELECT model_id, iteration_number, auc, leave_out_auc, serialized_confusion_matrix, serialized_assessment_set, serialized_word_scores, lexicon_id, (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Jerusalem') as created_at, model_type
    FROM model_run_info
...
getProjectConcepts <- function(project_id){
  ...
  concepts <- dbGetQuery(appDB, paste("SELECT id, name, description, rule, project_id, (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Jerusalem') as created_at 
